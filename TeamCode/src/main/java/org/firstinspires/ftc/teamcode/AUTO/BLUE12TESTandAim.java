...
@Autonomous
public class BLUE12TESTandAim extends CommandOpMode {
    ...
    public static class AimSubsystem extends SubsystemBase {
        private final Follower follower;
        private final TurretSubsystem turretSub;

        private double dist = 0;
        private double turretAng = 0;
        private double hoodAngle = 0;
        private double targetRPM = 0;

        private boolean turretInRange = false;

        public AimSubsystem(Follower follower, TurretSubsystem turretSub) {
            this.follower = follower;
            this.turretSub = turretSub;
        }

        /** Call this method continuously from a command to aim the turret */
        public void updateAim() {
            ...
            // 1. Calculate standard stationary angle
            ...
            // 2. Calculate target RPM and Hood angle based on distance
        ...
        private void applyToServos() {
            // Safety limit: Don't wrap turret cables
            ...
            // Map math angle to servo degrees (180 is center)
            double set = MathFunctions.clamp(180 + (turretAng * 3.2) / 2, 0, 360);

            turretSub.turret1.set(set);
            turretSub.turret2.set(set);
            turretSub.hood.set(MathFunctions.clamp(hoodAngle, 0, 300));
        }
    ...
    public static class ContinuousAimCommand extends CommandBase {
        private final AimSubsystem aimSub;

        public ContinuousAimCommand(AimSubsystem aimSub) {
            this.aimSub = aimSub;
            // Requiring AimSubsystem ensures no other command can accidentally
            // fight for control of the turret servos simultaneously.
            addRequirements(aimSub);
        }
    ...
    public void initialize() {
        ...
        AimSubsystem aimSub = new AimSubsystem(follower, turretSub);


        register(intakeSub, turretSub, launcherSub,aimSub);
